...
#GAME NAME:	This specifies the name of the game text is being dumped from.
		The name specified here will be included in the output file. This
...
#LINE LENGTH:	Similar to #STRING LENGTH, this command specifies the length of
		a line in bytes. Once the number of bytes specified have been
...
#METHOD:	This allows the user to specify one of three ways to dump the text:
		...
		RAW:
			A simple start to stop dump, like Rom juice.

#POINTER ENDIAN:
		Specifies the endianness of the pointers: BIG or LITTLE
...
End tokens specified by beginning the entry with a '/' (like /FF=<END>\n\n above) are used
